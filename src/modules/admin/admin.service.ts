...
export class AdminService{
  ...
  async ensureIsAdmin(id: string): Promise<Boolean>{
    const admin = await this.adminRepository.exists({userType:UserType.ADMIN, _id: id});
  ...
  async createAdmin({email, name, password}: CreateAdminDto): Promise<Admin>{
    ...
    const admin = await this.adminRepository.create({email, name, password:hashedPassword, userType: UserType.ADMIN});
    return admin
  }
